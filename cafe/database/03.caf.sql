...
CREATE TABLE employees (
	...
	pay_scale_id			integer references pay_scales not null,
	...
	normal_work_hours		real default 9 not null CHECK (normal_work_hours > 1),
...
CREATE TABLE intake (
	...
	employee_type_id		integer references employee_types,
	...
	contract				boolean default false not null,
	contract_period			integer not null,
	unsuccessful_email		text,
	details					text
);
CREATE INDEX intake_employee_type_id ON intake (employee_type_id);
...
CREATE TABLE interviews (
	...
	not_attended			boolean default false not null,
...
CREATE VIEW vw_locations AS
	SELECT orgs.org_id, orgs.org_name, 
		locations.location_id, locations.location_name, locations.latitude, locations.longitude, 
		locations.geo_position, locations.details,
		
		(CASE WHEN locations.geo_position is null THEN 'No Location' ELSE
			('<a href="https://www.google.com/maps/search/?api=1&query=' 
			|| locations.geo_position[0] || ',' || locations.geo_position[1]
			|| '" target="_blank">Map</a>') END) as location_map
		
	FROM locations INNER JOIN orgs ON locations.org_id = orgs.org_id;
...
CREATE VIEW vw_employees AS
	SELECT employee_types.employee_type_id, employee_types.employee_type_name,
		vw_department_roles.department_role_id, vw_department_roles.department_role_name, 
		vw_bank_branch.bank_id, vw_bank_branch.bank_name, vw_bank_branch.bank_branch_id, vw_bank_branch.bank_branch_name, 
		...
		orgs.org_id, orgs.org_name, orgs.org_full_name, orgs.pin, orgs.cert_number,
		employees.entity_id, employees.employee_id, employees.surname, employees.first_name, employees.middle_name, 
		...
		vw_education_max.name_of_school, vw_education_max.examination_taken, 
		vw_education_max.grades_obtained, vw_education_max.certificate_number
	FROM employees INNER JOIN  employee_types ON employees.employee_type_id = employee_types.employee_type_id
		INNER JOIN vw_department_roles ON employees.department_role_id = vw_department_roles.department_role_id
		INNER JOIN vw_bank_branch ON employees.bank_branch_id = vw_bank_branch.bank_branch_id		
		...
		INNER JOIN orgs ON employees.org_id = orgs.org_id
...
CREATE VIEW vw_intake AS
	SELECT employee_types.employee_type_id, employee_types.employee_type_name, 
		vw_department_roles.department_id, vw_department_roles.department_name, vw_department_roles.department_description, 
		...
		intake.contract_period, intake.unsuccessful_email, intake.details,
	...
	FROM intake INNER JOIN employee_types ON intake.employee_type_id = employee_types.employee_type_id
		INNER JOIN vw_department_roles ON intake.department_role_id = vw_department_roles.department_role_id
...
CREATE VIEW vw_applications AS
	SELECT vw_intake.department_id, vw_intake.department_name, vw_intake.department_description, vw_intake.department_duties,
		...
		vw_intake.org_name, vw_intake.unsuccessful_email, vw_intake.org_details,
...
CREATE OR REPLACE FUNCTION get_location_id(int, point) RETURNS integer AS $$
	SELECT location_id
	FROM locations
	WHERE (geo_position is not null) AND (org_id = $1)
	AND (length(lseg(geo_position, $2)) = 
		(SELECT min(length(lseg(geo_position, $2))) FROM locations
		WHERE (geo_position is not null) AND (org_id = $1)));
$$ LANGUAGE SQL;

CREATE OR REPLACE FUNCTION ins_locations() RETURNS trigger AS $$
DECLARE
BEGIN
	
	IF((NEW.latitude is not null) AND (NEW.longitude is not null))THEN
		NEW.geo_position := (NEW.latitude::text || ',' || NEW.longitude)::point;
	END IF;
...
CREATE TRIGGER ins_locations BEFORE INSERT OR UPDATE ON locations
    FOR EACH ROW EXECUTE PROCEDURE ins_locations();
...
BEGIN
	...
	ELSIF ($3 = '3') THEN
		...
		FROM applications WHERE (application_id = $1::int);

		SELECT sys_email_id INTO v_sys_email_id
		FROM sys_emails 
		WHERE (use_type = 15) AND (org_id = v_org_id);
		
		SELECT sys_emailed_id INTO v_sys_emailed_id
		FROM sys_emailed WHERE (sys_email_id = v_sys_email_id) AND (table_id = v_application_id);

		IF(v_sys_emailed_id is null)THEN
			INSERT INTO sys_emailed (sys_email_id, org_id, table_id, table_name, email_type)
			VALUES (v_sys_email_id, v_org_id, v_application_id, 'applications', 15);
		END IF;
		
		msg := 'Sent unsuccessful email';
	END IF;
...
BEGIN
	...
	IF(v_employee_id is null) AND (v_entity_id is null)THEN	
		INSERT INTO employees (org_id, currency_id, bank_branch_id,
			department_role_id, employee_type_id, 
			pay_scale_id, pay_group_id, location_id,  
		...
		SELECT orgs.org_id, orgs.currency_id, 0,
			intake.department_role_id, intake.employee_type_id, 
			intake.pay_scale_id, intake.pay_group_id, intake.location_id,
...
BEGIN
	...
	INSERT INTO sys_emailed (sys_email_id, org_id, table_id, table_name, email_type)
	VALUES (v_sys_email_id, v_org_id, v_interview_id, 'interviews', 14);
