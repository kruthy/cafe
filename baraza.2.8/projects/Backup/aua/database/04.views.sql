...
CREATE OR REPLACE VIEW vw_sub_levels AS
...
CREATE OR REPLACE VIEW vw_major_options AS
...
CREATE OR REPLACE VIEW vw_sponsors AS
	...
	FROM sponsor_types
		...
		LEFT JOIN entitys ON sponsors.entity_id = entitys.entity_id
...
CREATE OR REPLACE VIEW vw_transfered_credits AS 
	...
	--view  primary_majors--
CREATE OR REPLACE VIEW vw_primary_majors AS
	SELECT schools.school_id, schools.school_name, departments.department_id, departments.department_name, 
...
CREATE OR REPLACE VIEW vw_students_primary_majors AS
	SELECT students.org_id, students.student_id, students.student_name, students.account_number, students.Nationality, 
		...
		vw_primary_majors.major_id, vw_primary_majors.major_name,sstudents.no_approval,sstudents.sstudent_id
	FROM (students 
		...
		INNER JOIN sstudents ON sstudents.student_degree_id=vw_primary_majors.student_degree_id
	WHERE (student_degrees.completed = false);

	
	--view major opt_contents--
CREATE OR REPLACE VIEW vw_major_opt_contents AS
...
CREATE OR REPLACE VIEW vw_sstudents AS
	SELECT vw_student_degrees.religion_id, vw_student_degrees.religion_name, vw_student_degrees.denomination_id, 
		...
		sstudents.first_instalment,sstudents.first_date,sstudents.second_instalment,sstudents.second_date,sstudents.LRF_picked,sstudents.LRF_picked_date,sstudents.on_attachment,
...
SELECT vw_sstudents.religion_id, vw_sstudents.religion_name, vw_sstudents.denomination_id, vw_sstudents.denomination_name,
		...
		vw_sstudents.withdraw_date, vw_sstudents.withdraw_rate,  vw_sstudents.exchange_rate,vw_sgrade.school_id as crs_school_id,
		vw_sgrade.school_name as crs_school_name,
		...
		vw_sgrade.department_change, vw_sgrade.registry_change,vw_sgrade.clinical_fee, vw_sgrade.extra_charge,
		...
		(CASE WHEN vw_sgrade.examinable = true THEN vw_sstudents.exam_fees 
		ELSE 0 END)  as exam_fee,
		(CASE WHEN (vw_sgrade.lab_course = true) THEN vw_sstudents.lab_charges ELSE 0 END) as lab_fees
		

FROM vw_sstudents INNER JOIN vw_sgrade ON vw_sgrade.sstudent_id=vw_sstudents.sstudent_id;
...
CREATE OR REPLACE VIEW vw_scourse_check_pass AS
	SELECT vw_course_check_lists.order_id, vw_course_check_lists.student_id, vw_course_check_lists.student_degree_id, vw_course_check_lists.degree_id, vw_course_check_lists.description,
		...
		vw_scourses.no_gpa, vw_scourses.year_taken, 
		vw_scourses.instructor_id, vw_scourses.session_id, vw_scourses.scourse_id, vw_scourses.class_option, vw_scourses.max_class,
...
CREATE OR REPLACE VIEW vw_major_grade AS
...
CREATE OR REPLACE VIEW vw_sstudent_majors AS 
	SELECT vw_sstudents.session_id,vw_student_majors.major_id, vw_student_majors.sex, vw_sstudents.sstudent_id,major_name,
		...
		vw_student_majors.sub_level_name,vw_student_majors.primary_major,vw_student_majors.major,vw_student_majors.student_major_id,vw_sstudents.approved,vw_sstudents.curr_balance,vw_sstudents.no_approval,
		vw_sstudents.active,vw_sstudents.finalised,vw_sstudents.major_approval,vw_sstudents.degree_level_id
	FROM vw_sstudents
	INNER JOIN vw_student_majors ON vw_student_majors.degree_id = vw_sstudents.degree_id;


CREATE OR REPLACE VIEW vw_residence_rooms AS
...
CREATE OR REPLACE VIEW vw_sresidence_room AS
...
CREATE OR REPLACE VIEW vw_sstudent_res_room AS
...
CREATE OR REPLACE VIEW vw_sstudent_scharges AS 
...
CREATE OR REPLACE VIEW vw_sstudent_balances AS
...
CREATE OR REPLACE VIEW vw_current_residence AS 
